...
def train(model, opt):
    ...
    metadata = data.CIFAR100
    preprocess_train = metadata.train_transform()
    preprocess_test = metadata.test_transform()

    # make CIFAR loader:
    cifar100_train = datasets.CIFAR100('./data', train=True, download=True, transform=preprocess_train)
    cifar100_test = datasets.CIFAR100('./data', train=False, download=True, transform=preprocess_test)

    trainloader = DataLoader(cifar100_train, batch_size=BATCH_SIZE) 
    testloader = DataLoader(cifar100_test, batch_size=VAL_BATCH_SIZE)
