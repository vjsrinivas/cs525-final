...
# load in msa layers:
sys.path.append('../../classifiers/darknet/')
import msa
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        # generate MSA layer:
        if m == 'MSA': m = msa.AttentionBasicBlockA
        # traditional layers:
        else: m = eval(m) if isinstance(m, str) else m  # eval strings
