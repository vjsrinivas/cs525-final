...
# degrade functions:
from degradeTransforms import saltpepper
...
def applyDegrade(databatch, noise, param):
    #import matplotlib
    #matplotlib.use('TkAgg')
    rectified_batch = torch.transpose(databatch, 1, 2)
    rectified_batch = torch.transpose(rectified_batch, 2, 3)
    for i in range(rectified_batch.shape[0]):
        rectified_batch[i] = noise(rectified_batch[i], param)
    rectified_batch = torch.transpose(rectified_batch, 3,2)
    rectified_batch = torch.transpose(rectified_batch, 1,2)

    # for i in range(rectified_batch.shape[0]):
    #     plt.imshow(rectified_batch[i])
    #     plt.show()
    #     plt.close()
    # exit()
...
def run(data,
    ...
    # Configure
    model.eval()
    noise_params = [0,0.001, 0.01, 0.1] # TODO: REPLACE WITH YAML READINS
    mAP50s = []
    ...
    for noise_param in noise_params:
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            t1 = time_sync()
            im = applyDegrade(im, saltpepper, noise_param) #replace with yaml read in
    ...
    #record and save:
    with open( os.path.join(save_dir, 'maps.txt'), 'w') as f:
        for mp in mAP50s:
            f.write('%0.4f\n'%(mp))
...
def parse_opt():
    ...
    parser.add_argument('--noise', type=str, default='salt_pepper', help='Type of noise to apply to an image')
    ...
    parser.add_argument('--project', default=ROOT / 'runs/degrade', help='save to project/name')
